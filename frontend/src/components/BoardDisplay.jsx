...
const BoardDisplay = ({ board, onCloseBoard }) => {
  ...
  const updateBoard = async () => {
    try {
      await axios.put(
        `https://calikan.onrender.com/api/v1/boards/${board.boardId}`,
        { name: editedName }
      );
      setEditMode(false);
  ...
  const deleteBoard = async () => {
    try {
      await axios.delete(
        `http://localhost:8000/api/v1/boards/${board.boardId}`
      );
      onCloseBoard();
    } catch (error) {
  ...
  return (
    <div className=" bg-gray-300 w-[76rem] h-[38rem] flex justify-center rounded mt-6">
      ...
      <div className="fixed top-[14rem] right-[22rem]">
        <button onClick={onCloseBoard}>
          <BsBackspaceFill size={20} />
        </button>
      </div>
