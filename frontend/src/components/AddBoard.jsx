...
const AddBoard = ( { closeModal } ) => {
  ...
  const handleAddBoard = async () => {
    try {
      const response = await axios.post('http://localhost:8000/api/v1/boards', {
        name: boardName,
  ...
  return (
    <div className='fixed bg-gray-300 w-[30rem] h-[35rem] top-[21%] left-[39%] px-10 pt-5'>
        <div className='flex justify-between'>
          <h2 className='text-[#0e0e0e] font-bold text-lg cursor-pointer'>Add New Board</h2>
          <button onClick={closeModal}><BsFillBackspaceFill size={20}/></button>
        </div>
        <div className="mb-4">
          <label htmlFor="boardName" className="text-[#0e0e0e] block font-medium mb-1 mt-[3rem]">
            Board Name
          </label>
          <input
            type="text"
